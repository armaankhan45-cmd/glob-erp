...
/* ═══════════════════════════════════════════════════════════════
   CLASSIC LAYOUT — Original bordered box format
   All text BLACK (#000) for print visibility
   Subtotal row REMOVED per user request
   ═══════════════════════════════════════════════════════════════ */
function ClassicLayout({ quotation, items, org, boldOn, customerSize, qNum, gstRate, totalGST, subtotal, totalAmount, amountWords, letterheadMm, footerMm, selectedFont, selectedFontSize }) {
  ...
  return (
    <div className="bg-white shadow-lg mx-auto print-area" style={{
      ...
      {/* QUOTATION TITLE */}
      <div style={{ margin: '0 10mm', textAlign: 'center', padding: '10px 0 8px', fontSize: '16pt', fontWeight: 'bold', letterSpacing: '1px', color: '#000' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', border: '2px solid #000', margin: '0 10mm', overflow: 'hidden' }}>
        {/* Customer name row */}
        <div style={{ padding: '8px 12px 6px', textAlign: 'left', borderBottom: B, background: '#f0f0f0' }}>
          ...
          {quotation.additional_info && <div style={{ fontSize: '9.5pt', marginTop: '3px', color: '#000', fontWeight: '600' }}>{quotation.additional_info}</div>}
          {quotation.customer_gstin && <div style={{ fontSize: '9pt', marginTop: '2px', color: '#000', fontWeight: '700' }}>GSTIN: {quotation.customer_gstin}</div>}
        </div>
        ...
        <div style={{ padding: '0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: selectedFontSize, tableLayout: 'fixed' }}>
            ...
            <thead>
              <tr>
                <th style={{ border: B, padding: '7px 5px', background: '#e0e0e0', textAlign: 'center', fontWeight: 'bold', fontSize: selectedFontSize, color: '#000' }}>SR No.</th>
                <th style={{ border: B, padding: '7px 5px', background: '#e0e0e0', textAlign: 'left', fontWeight: 'bold', fontSize: selectedFontSize, color: '#000' }}>Particulars</th>
                <th style={{ border: B, padding: '7px 5px', background: '#e0e0e0', textAlign: 'center', fontWeight: 'bold', fontSize: selectedFontSize, color: '#000' }}>Qty</th>
                <th style={{ border: B, padding: '7px 5px', background: '#e0e0e0', textAlign: 'right', fontWeight: 'bold', fontSize: selectedFontSize, color: '#000' }}>Rate (INR)</th>
                <th style={{ border: B, padding: '7px 5px', background: '#e0e0e0', textAlign: 'right', fontWeight: 'bold', fontSize: selectedFontSize, color: '#000' }}>Amount (INR)</th>
              </tr>
            </thead>
            <tbody>
              {items.map((item, i) => (
                <tr key={i}>
                  <td style={{ border: B, padding: '6px 5px', textAlign: 'center', fontWeight: '700', color: '#000' }}>{i + 1}</td>
                  <td style={{ border: B, padding: '6px 5px', lineHeight: '1.3', fontWeight: boldOn ? 'bold' : 'normal', whiteSpace: 'pre-line', color: '#000' }}>{item.description || ''}</td>
                  <td style={{ border: B, padding: '6px 5px', textAlign: 'center', fontWeight: '700', color: '#000' }}>{item.quantity}{item.unit && item.unit !== 'Unit' ? ` ${item.unit}` : ''}</td>
                  <td style={{ border: B, padding: '6px 5px', textAlign: 'right', fontWeight: '700', color: '#000' }}>₹{fmt(item.rate)}</td>
                  <td style={{ border: B, padding: '6px 5px', textAlign: 'right', fontWeight: 'bold', color: '#000' }}>₹{fmt(item.amount)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* Totals section — NO Subtotal row */}
        <div style={{ padding: '0', flexShrink: 0 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: selectedFontSize }}>
            <tbody>
              {gstRate > 0 && (
                <tr>
                  <td style={{ border: B, padding: '6px 10px', textAlign: 'left', fontWeight: '700', color: '#000' }}>GST: {gstRate}%</td>
                  <td style={{ border: B, padding: '6px 10px', textAlign: 'right', fontWeight: 'bold', color: '#000' }}>₹{fmt(totalGST)}</td>
                </tr>
              )}
              <tr style={{ background: '#f5f5f5' }}>
                <td colSpan={2} style={{ border: B, padding: '7px 10px', fontSize: '11pt', fontWeight: 'bold', color: '#000' }}>Total : {amountWords} ONLY</td>
              </tr>
              <tr style={{ background: '#e0e0e0' }}>
                <td style={{ border: B, padding: '8px 10px', textAlign: 'left', fontSize: '13pt', fontWeight: 'bold', color: '#000' }}>Total Amount</td>
                <td style={{ border: B, padding: '8px 10px', textAlign: 'right', fontSize: '13pt', fontWeight: 'bold', color: '#000' }}>₹{fmt(totalAmount)}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
/* ═══════════════════════════════════════════════════════════════
   PRO LAYOUT — With letterhead header, stamp, signature
   All text BLACK (#000) for print visibility
   Subtotal row REMOVED per user request
...
function ProLayout({ quotation, items, org, boldOn, customerSize, qNum, gstRate, totalGST, subtotal, totalAmount, amountWords, selectedFont, selectedFontSize, letterheadMm, footerMm }) {
  ...
  return (
    <div className="bg-white shadow-lg mx-auto print-area" style={{
      ...
      {hasLetterhead ? (
        <div style={{ padding: '10px 14px 8px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: `2px solid ${NAVY}` }}>
          ...
          <div style={{ flex: 1, textAlign: 'center' }}>
            <div style={{ fontSize: '17pt', fontWeight: 900, color: '#000', letterSpacing: 2, textTransform: 'uppercase', lineHeight: 1.2 }}>{companyName}</div>
            ...
            {org?.gstin && (
              <span style={{ display: 'inline-block', background: NAVY, color: '#fff', padding: '2px 10px', borderRadius: 3, fontSize: '8.5pt', fontWeight: 700, marginTop: 3, letterSpacing: 0.5 }}>GSTIN: {org.gstin}</span>
            )}
          </div>
        ...
        <div style={{ padding: '12px 14px 8px', textAlign: 'center', borderBottom: `2px solid ${NAVY}` }}>
          <div style={{ fontSize: '18pt', fontWeight: 900, color: '#000', letterSpacing: 2.5, textTransform: 'uppercase' }}>{companyName}</div>
          ...
          {org?.gstin && <span style={{ display: 'inline-block', background: NAVY, color: '#fff', padding: '2px 10px', borderRadius: 3, fontSize: '8.5pt', fontWeight: 700, marginTop: 3 }}>GSTIN: {org.gstin}</span>}
        </div>
      )}
      ...
      {/* ══ QUOTATION TITLE BAR ══ */}
      <div style={{ display: 'flex', background: '#f5f7fa', borderBottom: B, padding: '8px 14px', alignItems: 'center' }}>
        <div style={{ flex: 1 }}>
          <span style={{ fontSize: '14pt', fontWeight: 900, color: '#000', letterSpacing: 3 }}>QUOTATION</span>
        </div>
        <div style={{ textAlign: 'right', fontSize: '9pt', fontWeight: 700, color: '#000' }}>
          <div>No: <span style={{ color: '#000', fontSize: '11pt', fontWeight: 800 }}>{qNum}</span></div>
      ...
      <div style={{ display: 'flex', border: B, borderTop: 'none' }}>
        <div style={{ flex: 1, padding: '8px 14px', borderRight: '1px solid #ccc' }}>
          ...
          {quotation.additional_info && <div style={{ fontSize: '9pt', color: '#000', fontWeight: 600, marginTop: 2 }}>{quotation.additional_info}</div>}
        </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: selectedFontSize, border: B, borderTop: 'none' }}>
        ...
        <thead>
          <tr style={{ background: '#d5dae0' }}>
            <th style={{ padding: '7px 5px', border: bdr, fontWeight: 800, fontSize: selectedFontSize, color: '#000', textAlign: 'center' }}>#</th>
            <th style={{ padding: '7px 5px', border: bdr, fontWeight: 800, fontSize: selectedFontSize, color: '#000', textAlign: 'left' }}>DESCRIPTION</th>
            <th style={{ padding: '7px 5px', border: bdr, fontWeight: 800, fontSize: selectedFontSize, color: '#000', textAlign: 'center' }}>QTY</th>
            <th style={{ padding: '7px 5px', border: bdr, fontWeight: 800, fontSize: selectedFontSize, color: '#000', textAlign: 'center' }}>UNIT</th>
            <th style={{ padding: '7px 5px', border: bdr, fontWeight: 800, fontSize: selectedFontSize, color: '#000', textAlign: 'right' }}>RATE</th>
            <th style={{ padding: '7px 5px', border: bdr, fontWeight: 800, fontSize: selectedFontSize, color: '#000', textAlign: 'right' }}>AMOUNT</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item, i) => {
      ...
      {/* ══ TOTALS — NO Subtotal row ══ */}
      <div style={{ border: B, borderTop: 'none' }}>
        {gstRate > 0 && (
          <div style={{ display: 'flex', borderBottom: bdr }}>
            <div style={{ flex: 1, padding: '6px 14px', fontSize: selectedFontSize, fontWeight: 700, color: '#000' }}>GST: {gstRate}%</div>
            <div style={{ width: '180px', padding: '6px 14px', textAlign: 'right', fontSize: selectedFontSize, fontWeight: 700, color: '#000' }}>₹{fmt(totalGST)}</div>
          </div>
        )}
        <div style={{ background: '#f5f7fa', padding: '7px 14px', borderBottom: bdr }}>
          <div style={{ fontSize: selectedFontSize, fontWeight: 700, color: '#000' }}>Amount in Words: <span style={{ fontWeight: 800 }}>{amountWords} ONLY</span></div>
        </div>
        <div style={{ display: 'flex', background: '#d5dae0' }}>
          <div style={{ flex: 1, padding: '9px 14px', fontSize: '13pt', fontWeight: 900, color: '#000' }}>Total Amount</div>
          <div style={{ width: '180px', padding: '9px 14px', textAlign: 'right', fontSize: '13pt', fontWeight: 900, color: '#000' }}>₹{fmt(totalAmount)}</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', border: B, borderTop: 'none' }}>
        {/* Terms */}
        <div style={{ flex: 1, padding: '10px 14px', borderRight: '1px solid #ccc', fontSize: '9pt', lineHeight: 1.6, color: '#000', fontWeight: 600 }}>
          <div style={{ fontSize: '9pt', fontWeight: 800, color: '#000', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>Terms & Conditions</div>
        ...
        {(org?.bank_name || org?.account_no) && (
          <div style={{ width: '200px', padding: '10px 14px', borderRight: '1px solid #ccc', fontSize: '8.5pt', lineHeight: 1.6, color: '#000', fontWeight: 600 }}>
            <div style={{ fontWeight: 800, color: '#000', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5, fontSize: '9pt' }}>Bank Details</div>
        ...
        {/* Signature + Stamp */}
        <div style={{ flex: 1, padding: '10px 14px', textAlign: 'right', fontSize: '9pt', color: '#000' }}>
          <div style={{ fontSize: '9pt', fontWeight: 800, color: '#000', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>For {companyName}</div>
...
th,td { padding:6px 10px; }
  ...
  /* ═══ FONT & SPACING — Read from org (primary), then localStorage (fallback) ═══ */
