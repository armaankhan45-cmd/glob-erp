      ...
      await api.put('/invoices/' + id + '/full', {
        ...
        items: valid.map(function (it) {
          return {
            ...
            unit: it.unit, rate: parseFloat(it.rate) || 0, tax_rate: parseFloat(it.tax_rate) || 0
          }
        }),
      ...
      <form onSubmit={submit} className="space-y-5">
        <div className="glass rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><label className="block text-xs font-semibold text-slate-400 mb-1.5">Invoice Number *</label><input type="text" value={invoiceNumber} onChange={function (e) { setInvoiceNumber(e.target.value) }} required className="w-full px-3 py-2.5 rounded-xl text-sm font-mono font-bold" autoComplete="off" /></div>
              ...
              return (
                <div key={idx} className="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
                  ...
                  <div className="grid grid-cols-12 gap-2">
                    <div className="col-span-12 md:col-span-3"><label className="block text-xs text-slate-400 mb-1">Description *</label><input type="text" value={item.description} onChange={function (e) { updateItem(idx, 'description', e.target.value) }} className="w-full px-3 py-2 rounded-lg text-sm" autoComplete="off" /></div>
                    <div className="col-span-6 md:col-span-2"><label className="block text-xs text-slate-400 mb-1">HSN</label><input type="text" value={item.hsn_code} onChange={function (e) { updateItem(idx, 'hsn_code', e.target.value) }} className="w-full px-2 py-2 rounded-lg text-sm font-mono" autoComplete="off" /></div>
                    ...
                    <div className="col-span-6 md:col-span-2"><label className="block text-xs text-slate-400 mb-1">Rate *</label><input type="text" inputMode="decimal" value={item.rate} onChange={function (e) { updateItem(idx, 'rate', e.target.value) }} className="w-full px-3 py-2 rounded-lg text-sm" autoComplete="off" /></div>
                    <div className="col-span-6 md:col-span-1"><label className="block text-xs text-slate-400 mb-1">GST %</label><select value={item.tax_rate} onChange={function (e) { updateItem(idx, 'tax_rate', e.target.value) }} className="w-full px-2 py-2 rounded-lg text-sm font-bold">{GST_SLABS.map(function (s) { return <option key={s.value} value={s.value}>{s.value}%</option> })}</select></div>
