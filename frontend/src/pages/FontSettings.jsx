  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="card space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Print Font Family</label>
          <select value={fontFamily} onChange={e => setFontFamily(e.target.value)} className="input-field">
            {GOOGLE_FONTS.map(f => <option key={f} value={f}>{f}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Description Font Size: {fontSize}pt</label>
      ...
      <div className="card">
        ...
        <div className="border rounded-lg p-6 bg-white" style={{ fontFamily, fontSize: `${fontSize}pt` }}>
