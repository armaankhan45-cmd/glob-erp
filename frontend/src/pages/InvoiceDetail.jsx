...
function ProLayout({ inv, items, org, invNum, isPaid, placeOfSupply, invoiceDate, totalQty, isPrintIntraState, totalCgst, totalSgst, totalIgst, totalTax, hsnMap, qrUrl, custGstin, custStateCode, orgStateCode, fontFamily, boldOn, fmt }) {
  ...
  return (
    <div className="bg-white shadow-lg mx-auto print-area" style={{ fontFamily, maxWidth: 900, margin: '0 auto', background: '#fff', border: `2px solid ${NAVY}`, color: '#000', fontWeight: 600, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${NAVY} 0%, #1b2a4a 100%)`, padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 16 }}>
        <div style={{ width: 68, height: 68, flexShrink: 0, borderRadius: 6, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid rgba(255,255,255,0.3)' }}>
          {org.logo_url
        ...
        <div style={{ flex: 1 }}>
          <h1 style={{ fontSize: 20, fontWeight: 900, color: '#fff', letterSpacing: 2, lineHeight: 1.2, margin: '0 0 3px' }}>{(org.name || 'GLOB FABRICATION AND ENTERPRISES').toUpperCase()}</h1>
          {org.gstin && <span style={{ display: 'inline-block', background: 'rgba(255,255,255,0.15)', color: '#fff', padding: '2px 10px', borderRadius: 3, fontSize: 11, fontWeight: 700, letterSpacing: 0.5 }}>GSTIN: {org.gstin}</span>}
          <div style={{ fontSize: '11.5px', color: 'rgba(255,255,255,0.8)', marginTop: 2, lineHeight: 1.4 }}>{[org.address, org.city, org.state, org.pincode ? 'PIN: ' + org.pincode : ''].filter(Boolean).join(', ')}</div>
          <div style={{ fontSize: '11.5px', color: 'rgba(255,255,255,0.8)', marginTop: 1, lineHeight: 1.4 }}>{org.phone ? `📞 ${org.phone}` : ''}{org.email ? ` ✉ ${org.email}` : ''}</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 20, fontWeight: 900, color: '#fff', letterSpacing: 4 }}>TAX INVOICE</div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)', letterSpacing: 1, fontWeight: 600, marginTop: 2 }}>ORIGINAL FOR RECIPIENT</div>
        </div>
      </div>
