...
export default function PurchaseNew() {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await api.post('/purchases', { ...form, bill_date: form.bill_date || null, ...calculated, items: items.map(i => ({ ...i, quantity: parseFloat(i.quantity), rate: parseFloat(i.rate), amount: parseFloat(i.amount) })) })
      navigate('/app/purchases')
    } catch (err) { alert(err.response && err.response.data && err.response.data.msg || 'Failed') }
      ...
      <div className="card">
        ...
        {items.map((item, idx) => (
          <div key={idx} className="grid grid-cols-7 gap-2 mb-2 items-end">
            <div className="col-span-2"><ItemSuggestInput value={item.description} onChange={(val) => updateItem(idx, 'description', val)} onSelect={(suggested) => handleItemSuggest(idx, suggested)} placeholder="Description" className="input-field text-sm" /></div>
