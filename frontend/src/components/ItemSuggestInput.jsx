    ...
    <div ref={wrapperRef} style={{ position: 'relative' }}>
      ...
      {showDropdown && suggestions.length > 0 && (
        <div style={{
          ...
          maxHeight: '280px', overflowY: 'auto', background: 'var(--bg-card)',
          border: '1px solid var(--border-strong)', borderRadius: '8px',
          boxShadow: '0 8px 24px rgba(0,0,0,0.15)', marginTop: '2px',
        }}>
          {suggestions.map((item, idx) => (
            <div key={idx} onClick={() => handleSelect(item)} onMouseEnter={() => setHighlightIndex(idx)}
              style={{
                padding: '8px 12px', cursor: 'pointer', display: 'flex',
                alignItems: 'center', justifyContent: 'space-between', gap: '8px',
                background: idx === highlightIndex ? 'rgba(var(--accent-rgb),0.08)' : 'transparent',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                {item.hsn_code && (
                  <span style={{
                    ...
                    borderRadius: '4px', background: 'rgba(var(--accent-rgb),0.1)',
                    color: 'var(--accent)', fontFamily: 'monospace', letterSpacing: '0.5px',
                  }}>HSN {item.hsn_code}</span>
                )}
                {item.rate > 0 && (
                  <span style={{ fontSize: '10px', fontWeight: '600', color: 'var(--text-muted)' }}>₹{item.rate}</span>
                )}
                {item.unit && item.unit !== 'NOS' && (
                  <span style={{ fontSize: '10px', fontWeight: '600', color: 'var(--text-muted)' }}>/{item.unit}</span>
                )}
              </div>
            </div>
          ))}
