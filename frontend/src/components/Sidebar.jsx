  ...
  return (
    <>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-[280px] flex flex-col transition-transform duration-400 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
        style={{ background: 'var(--bg-sidebar)', backdropFilter: 'blur(40px)', borderRight: '1px solid var(--border)', transitionTimingFunction: 'cubic-bezier(0.16,1,0.3,1)' }}>
        ...
        <div className="p-3 space-y-2" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <button onClick={toggleMode}
            ...
            style={{
              ...
              color: mode === 'light' ? '#f59e0b' : 'var(--text-primary)'
            }}>
          ...
          <div className="p-3 rounded-xl" style={{ background: `rgba(${hexToRgb(accentColor)}, 0.04)`, border: `1px solid rgba(${hexToRgb(accentColor)}, 0.1)` }}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 font-extrabold text-sm"
                style={{ background: `linear-gradient(135deg, ${accentColor}, ${accentColor}cc)`, boxShadow: `0 0 12px rgba(${hexToRgb(accentColor)}, 0.25)`, color: '#fff' }}>
                {user?.name?.charAt(0)?.toUpperCase() || 'A'}
              </div>
              <div className="min-w-0 flex-1">
                <h4 className="text-[11px] font-bold leading-tight truncate" style={{ color: 'var(--text-primary)' }}>
