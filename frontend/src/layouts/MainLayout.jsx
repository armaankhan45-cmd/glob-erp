  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: '#080b14' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <TopBar />
        <main className="flex-1 overflow-y-auto p-4 md:p-6" style={{ background: '#080b14' }}><Outlet /></main>
      </div>
            ...
            <div ref={miniChatRef} className="fixed bottom-24 right-6 w-[380px] rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden"
              ...
              <div className="px-4 py-3 flex items-center justify-between flex-shrink-0" style={{ background: 'rgba(6,182,212,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => { setAiOpen(false); navigate('/app/ai-assistant') }} className="p-1.5 rounded-lg" style={{ color: 'rgba(255,255,255,0.4)' }}><Bot size={14} /></button>
                  <button onClick={() => setAiOpen(false)} className="p-1.5 rounded-lg" style={{ color: 'rgba(255,255,255,0.4)' }}><X size={14} /></button>
                </div>
              </div>
              ...
              <div className="flex gap-2 p-3 flex-shrink-0" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <button onClick={() => aiSend(aiInput)} disabled={aiLoading || !aiInput.trim()} className="p-2 rounded-xl text-white disabled:opacity-50" style={{ background: 'linear-gradient(135deg, #06b6d4, #a855f7)' }}><Send size={14} /></button>
              </div>
            </div>
          )}
          <button id="ai-float-btn" onClick={() => setAiOpen(!aiOpen)} className="fixed bottom-6 right-6 w-14 h-14 text-white rounded-full shadow-lg hover:scale-105 transition-all z-50 flex items-center justify-center"
            style={{ background: 'linear-gradient(135deg, #06b6d4, #a855f7, #4f8fff)', animation: 'pulseGlow 3s ease-in-out infinite' }}>
